...
function successResult(verb) {
  return {
    schemaVersion: 1,
    ok: true,
    cli: { name: "@taprootio/site-authoring", version: "0.10.1" },
    verb,
  };
}
...
test("exposes help and version at the binary and verb levels", async (testContext) => {
  ...
  for (
    ...
    assert.equal(versionStdout.read(), "0.10.1\n");
  }
});
...
test("serves page and component reference help without configuration, credentials, handlers, or network", async (context) => {
  const cases = [
    ...
    },
    {
      ...
      match: /^Espalier 4\.16\.0 complete site-theme contract/u,
    ...
    },
    {
      ...
      contains: [
        ...
        "an empty fontMenu falls back to the body font",
        "tokens --esp-color-headings",
        "chroma 0..0.25",
        "Atomic change set (one transaction)",
        "Dry run: taproot-site theme push --dry-run",
